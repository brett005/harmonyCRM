...
$(document).ready(function() {
    ...
    if (typeof(user_sip_conf) === 'undefined') {
        user_sip_conf = JSON.parse(localStorage.getItem('SIPCreds'));
    }

    ctxSip = {

        config : {
            password        : user_sip_conf.Pass,
            displayName     : user_sip_conf.Display,
            uri             : 'sip:'+user_sip_conf.User+'@'+user_sip_conf.Realm,
            wsServers       : user_sip_conf.WSServer,
        ...
        newSession : function(newSess) {
            ...
            newSess.displayName = newSess.remoteIdentity.displayName || newSess.remoteIdentity.uri.user_sip_conf;
        ...
        logItem : function(item) {
            ...
            if (callActive) {
                ...
                if (item.status === 'ringing' && item.flow === 'incoming') {
                    i += '<button class="btn btn-xs btn-success btnCall" title="Call"><i class="fa fa-phone"></i>Hezz</button>';
                } else {
